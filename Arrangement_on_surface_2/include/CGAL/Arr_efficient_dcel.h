#ifndef CGAL_ARR_EFFICIENT_DCEL_H
#define CGAL_ARR_EFFICIENT_DCEL_H
 ...
 * Base vertex class.
 */
template <typename Point_> class Arr_eff_vertex_base {
public:
   ...
   */
  template<typename PNT> struct rebind { typedef Arr_eff_vertex_base<PNT> other; };
...
public:
  ...
  { const_cast<Arr_eff_vertex_base&>(*this).p_inc = inc; }
  ...
  virtual void assign(const Arr_eff_vertex_base<Point>& v)
  {
    p_pt = v.p_pt;
    pss[0] = v.pss[0];
    pss[1] = v.pss[1];
 ...
 * Base halfedge class.
 */
template <typename X_monotone_curve_> class Arr_eff_halfedge_base {
public:
  ...
  struct rebind { typedef Arr_eff_halfedge_base<XCV> other; };
...
public:
  ...
  void set_curve(X_monotone_curve* c)
  {
    ...
    Arr_eff_halfedge_base<X_monotone_curve>* opp =
      reinterpret_cast<Arr_eff_halfedge_base<X_monotone_curve>* >(p_opp);

    opp->p_cv = c;
  }

  /*! Assign from another halfedge. */
  virtual void assign(const Arr_eff_halfedge_base<X_monotone_curve>& he)
  { p_cv = he.p_cv; }
};
...
public:
  /*! Default constructor. */
  Arr_eff_face_base() : flags(0) {}

  /*! Destructor. */
  virtual ~Arr_eff_face_base() {}
  ...
  virtual void assign(const Arr_eff_face_base& f) { flags = f.flags; }
};

// Forward declarations:
template <class V, class H, class F> class Arr_eff_vertex;
template <class V, class H, class F> class Arr_eff_halfedge;
template <class V, class H, class F> class Arr_eff_face;
template <class V, class H, class F> class Arr_eff_outer_ccb;
template <class V, class H, class F> class Arr_eff_inner_ccb;
template <class V, class H, class F> class Arr_eff_isolated_vertex;

/*! \class
 * The default arrangement DCEL vertex class.
 */
template <class V, class H, class F>
class Arr_eff_vertex : public V
{
public:

  typedef V                           Base;
  typedef Arr_eff_vertex<V,H,F>           Vertex;
  typedef Arr_eff_halfedge<V,H,F>         Halfedge;
  typedef Arr_eff_isolated_vertex<V,H,F>  Isolated_vertex;
 ...
 */
template <class V, class H, class F>
class Arr_eff_halfedge : public H
{
public:
  typedef H                     Base;
  typedef Arr_eff_vertex<V,H,F>     Vertex;
  typedef Arr_eff_halfedge<V,H,F>   Halfedge;
  typedef Arr_eff_face<V,H,F>       Face;
  typedef Arr_eff_outer_ccb<V,H,F>  Outer_ccb;
  typedef Arr_eff_inner_ccb<V,H,F>  Inner_ccb;
 ...
 */
template <class V, class H, class F>
class Arr_eff_face : public F, public Compact_container_base
{
public:
  typedef F                            Base;
  typedef Arr_eff_vertex<V,H,F>            Vertex;
...
public:
  ...
  Inner_ccb_iterator splice_inner_ccbs(Arr_eff_face& other)
  {
  ...
  Isolated_vertex_iterator splice_isolated_vertices(Arr_eff_face& other)
  {
 ...
 */
template <class V, class H, class F>
class Arr_eff_outer_ccb
{
public:
  typedef Arr_eff_outer_ccb<V,H,F>               Self;
  typedef Arr_eff_halfedge<V,H,F>                Halfedge;
  typedef Arr_eff_face<V,H,F>                    Face;
...
public:
  /*! Default constructor. */
  Arr_eff_outer_ccb() : p_f(nullptr), iter_is_not_singular(false) {}

  /*! Copy constructor. */
  Arr_eff_outer_ccb(const Arr_eff_outer_ccb& other) :
 ...
 */
template <class V, class H, class F>
class Arr_eff_inner_ccb : public Compact_container_base
{
public:
  typedef Arr_eff_inner_ccb<V,H,F>               Self;
  typedef Arr_eff_halfedge<V,H,F>                Halfedge;
  typedef Arr_eff_face<V,H,F>                    Face;
...
private:
  union
  {
    ...
    Arr_eff_inner_ccb* icc;       // next inner CCB in chain to valid icc
  } f_or_icc;
...
public:
  /*! Default constructor. */
  Arr_eff_inner_ccb() : status(ITER_IS_SINGULAR) { f_or_icc.f = nullptr; }

  /*! Copy constructor. */
  Arr_eff_inner_ccb(const Arr_eff_inner_ccb& other) :
  ...
  void set_next(Arr_eff_inner_ccb* next)
  {
    status = INVALID;
    f_or_icc.icc = next;
  }
 ...
 */
template <class V, class H, class F>
class Arr_eff_isolated_vertex
{
public:
  typedef Arr_eff_isolated_vertex<V,H,F>                Self;
  typedef Arr_eff_face<V,H,F>                           Face;
...
public:
  /*! Default constructor. */
  Arr_eff_isolated_vertex() : p_f(nullptr), iter_is_not_singular(false) {}

  /*! Copy constructor. */
  Arr_eff_isolated_vertex(const Arr_eff_isolated_vertex& other) :
    p_f(other.p_f), iter_is_not_singular(other.iter_is_not_singular)
 ...
 */
template <class V, class H, class F>
class Arr_efficient_dcel {
public:
  // Define the vertex, halfedge and face types.
  typedef Arr_efficient_dcel<V,H,F>        Self;
  typedef Arr_eff_vertex<V,H,F>           Vertex;
  typedef Arr_eff_halfedge<V,H,F>         Halfedge;
  typedef Arr_eff_face<V,H,F>             Face;
  typedef Arr_eff_outer_ccb<V,H,F>        Outer_ccb;
...
protected:
  typedef Compact_container<Vertex>           Vertex_list;
  typedef Compact_container<Halfedge>         Halfedge_list;
  typedef Compact_container<Face>             Face_list;
