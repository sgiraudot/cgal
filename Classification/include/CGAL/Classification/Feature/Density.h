...
#ifndef CGAL_CLASSIFICATION_FEATURE_DENSITY_H
#define CGAL_CLASSIFICATION_FEATURE_DENSITY_H
...
class Density : public Feature_base
{
...
public:
  ...
  */
  Density (const PointRange& input,
           PointMap point_map,
           const Grid& grid)
    : input(input), point_map(point_map), grid(grid)
  {
    ...
    double area = grid.resolution() * grid.resolution();

    for (std::size_t j = 0; j < grid.height(); ++ j)
      for (std::size_t i = 0; i < grid.width(); ++ i)
        if (grid.has_points(i,j))
        {
          std::size_t nb_pts = std::distance (grid.indices_begin(i,j), grid.indices_end(i,j));
    ...
    if (grid.width() * grid.height() > input.size())
    {
      ...
      for (std::size_t i = 0; i < input.size(); ++ i)
      {
        ...
        values[i] = internal_float(dtm(I,J) - get (point_map, *(input.begin() + i)).z());
      }
      dtm.free();
    }
...
#endif // CGAL_CLASSIFICATION_FEATURE_DENSITY_H
